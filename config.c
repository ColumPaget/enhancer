...
enhancer_settings_struct enhancer_settings;
int line=1;
...
void enhancer_config_destroy(TEnhancerConfig *Config)
{
    ...
    if (Config->Matches) destroy(Config->Matches);
    if (Config->Actions) destroy(Config->Actions);
    destroy(Config);
}
...
int enhancer_config_matches(TEnhancerConfig *Config, int MatchInt, const char *MatchStr)
{
    ...
    for (i=0; i < Config->NoOfMatches; i++)
    {
        ...
        case MATCH_FAMILY:
            ...
            {
            case FAMILY_UNIX:
                if (MatchInt==AF_UNIX) return(TRUE);
                break;
            case FAMILY_NETLINK:
                if (MatchInt==AF_NETLINK) return(TRUE);
                break;
            case FAMILY_NET:
                if ((MatchInt==AF_INET) || (MatchInt==AF_INET6)) return(TRUE);
                break;
            case FAMILY_IP4:
                if (MatchInt==AF_INET) return(TRUE);
                break;
            case FAMILY_IP6:
                if (MatchInt==AF_INET6) return(TRUE);
                break;
            }
            break;
